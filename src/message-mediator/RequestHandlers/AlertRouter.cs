...
public class TemperatureTooLowAlertRouter(DaprClient daprClient, EventConsumedMetrics eventConsumedMetrics, ILogger<DeviceRegistrationHandler> logger) : IRequestHandler<TemperatureTooLowAlert, MessageHandlingResult>
{
    public async Task<MessageHandlingResult> Handle(TemperatureTooLowAlert message, CancellationToken cancellationToken)
    {
        try
        {
            await daprClient.PublishEventAsync("commands", "commands", new IncreaseHeating(message.DeviceId));
            logger.LogInformation("Temperature Too Low Alert raised for device {DeviceId}, sent increase heating command", message.DeviceId);
        }
        catch (Exception e)
        {
            logger.LogError(e, "Error publishing command");
