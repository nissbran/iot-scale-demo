...
{
    private readonly IMediator _messageMediator;
    ...
    public IotEventsConsumer(ConsumerConfig config, IMediator messageMediator, EventConsumedMetrics metrics, int number, string topic, int commitPeriod = 1)
    {
            ...
            try
            {
                ...
                switch (typedMessage)
                {
                    case TelemetryMessage telemetryMessage:
                        await _messageMediator.Publish(telemetryMessage, cancellationToken);
                        break;
                    case CommandMessage commandMessage:
                        await _messageMediator.Send(commandMessage, cancellationToken);
                        break;
                }
